"""Resource handlers for Bynder Connector."""
...
@chat.function(
    "list_assets",
    "List digital assets in Bynder DAM.",
    action_type="read",
    chain_callable=True,
    event="bynder-connector.list_assets",
    effects=["read:assets"],
...
async def list_assets(ctx, params: ListAssetsParams) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.error(f"Error listing Bynder assets: {e}")

@chat.function(
    "get_asset",
    "Get details of one digital asset in Bynder.",
    action_type="read",
    chain_callable=True,
    event="bynder-connector.get_asset",
    effects=["read:asset"],
...
@chat.function(
    "list_collections",
    "List albums or collections in Bynder.",
    action_type="read",
    chain_callable=True,
    event="bynder-connector.list_collections",
...
@chat.function(
    "audit_dam_health",
    "Audit Bynder digital asset library health and type distribution.",
    action_type="read",
    chain_callable=True,
    event="bynder-connector.audit_dam_health",
...
async def audit_dam_health(ctx, params: ListAssetsParams) -> ActionResult:
    ...
    try:
        ...
        rec = DamHealthRecord(
            ...
            summary=f"Bynder library contains {len(assets)} sampled assets across {len(types)} media formats."
        )
