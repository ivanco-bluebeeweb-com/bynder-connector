"""Connection lifecycle for Bynder Connector."""
...
async def connect_bynder(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Bynder Connector."""
    client = BynderClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Bynder Account",
...
@chat.function(
    "list_connections",
    "List connected Bynder accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
async def disconnect_bynder(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
