"""Connection management for Bynder Connector."""
...
from bynder_client import BynderClient

async def resolve_client(ctx, connection_id: str = "") -> BynderClient:
    connections = (await ctx.store.get("connections", [])) or []
    if not connections:
        raise ValueError("No Bynder connections configured. Use connect_bynder first.")
    conn = None
    if connection_id:
        ...
        conn = connections[0]
    return BynderClient(auth_token=conn["auth_token"], base_url=conn.get("base_url", ""))

@chat.function(
    "connect_bynder",
    "Connect Bynder account via credentials.",
    action_type="write",
    chain_callable=True,
    event="bynder-connector.connect_bynder",
    effects=["create:connection"],
    data_model=ConnectionRecord
)
async def connect_bynder(ctx, params: ConnectParams) -> ActionResult:
    """Connect Bynder DAM account."""
    client = BynderClient(auth_token=params.auth_token, base_url=params.base_url)
    res = await client.verify_auth()
    if res.get("status") == "error":
        return ActionResult.error(f"Failed to authenticate with Bynder: {res.get('error')}")

    connections = (await ctx.store.get("connections", [])) or []
    masked = params.auth_token[:6] + "..." if len(params.auth_token) > 6 else "***"
    record = {
        "id": f"conn_{uuid.uuid4().hex[:8]}",
        "label": params.label or "Primary Bynder",
    ...
    return ActionResult.success(
        ...
        summary=f"Connected Bynder account '{record['label']}'."
    )

@chat.function(
    "list_connections",
    "List configured Bynder connections.",
    action_type="read",
    chain_callable=True,
    event="bynder-connector.list_connections",
...
@chat.function(
    "disconnect_bynder",
    "Disconnect Bynder account and delete stored credentials.",
    action_type="destructive",
    chain_callable=True,
    event="bynder-connector.disconnect_bynder",
    effects=["delete:connection"],
    data_model=DeleteResult
)
async def disconnect_bynder(ctx, params: ConnectionIdParams) -> ActionResult:
    """Disconnect account."""
    connections = (await ctx.store.get("connections", [])) or []
    if not connections:
    ...
    return ActionResult.success({"success": True, "message": f"Disconnected connection {target_id}."}, summary=f"Disconnected Bynder connection.")
